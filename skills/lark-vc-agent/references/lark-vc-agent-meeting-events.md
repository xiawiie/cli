...
转发到 IM 时使用 `+meeting-events --format json` 的结构化事件，不要解析 pretty/Markdown 输出。对 `event_type == "chat_received"` 的事件逐项处理 `payload.chat_received_items`：

- `message_type == 3` 是会中 reaction；把同一 item 的 `content` 填入 Feishu `post` 节点的 `emoji_type`，节点形态是 `{"tag":"emotion","emoji_type":"<content>"}`。
- 其他聊天消息按文本节点发送，节点形态是 `{"tag":"text","text":"<content>"}`。
